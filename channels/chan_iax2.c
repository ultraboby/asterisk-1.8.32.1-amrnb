...
static unsigned int calc_timestamp(struct chan_iax2_pvt *p, unsigned int ts, struct ast_frame *f)
{
	...
	*/
	...
	if (delivery && !ast_tvzero(*delivery)) {
		...
		// if delivery is 10 minutes or more smaller than p->offset, then delivery (RTP timestamp derived) has wrapped, so add offset of 536870912ms (approx 6.2 days) to ms : 
		if (ms < -600000) {
			ms = ms + 536870912;
		} else if (ms < 0) {
			ms = 0;
