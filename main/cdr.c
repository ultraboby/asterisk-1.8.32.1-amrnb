...
void ast_cdr_getvar(struct ast_cdr *cdr, const char *name, char **ret, char *workspace, int workspacelen, int recur, int raw)
{
	...
	*ret = NULL;
	...
	} else if (!strcasecmp(name, "rduration"))
		snprintf(workspace, workspacelen, "%f", cdr->rduration);
	else if (!strcasecmp(name, "rbillsec"))
		snprintf(workspace, workspacelen, "%f", cdr->rbillsec);
...
static const char * const cdr_readonly_vars[] = { "clid", "src", "dst", "dcontext", "channel", "dstchannel",
						  "lastapp", "lastdata", "start", "pdd", "answer", "end", "pddsec", "duration",
						  "billsec", "disposition", "amaflags", "accountcode", "uniqueid", "linkedid",
						  "userfield", "sequence", "rduration", "rbillsec", NULL };
...
void ast_cdr_merge(struct ast_cdr *to, struct ast_cdr *from)
{
	...
	if (!ast_tvzero(from->pdd)) {
		if (!ast_tvzero(to->pdd)) {
			if (ast_tvcmp(to->pdd, from->pdd) > 0 ) {
				to->pdd = from->pdd; /* use the earliest time */
				from->pdd = ast_tv(0,0); /* we actively "steal" these values */
			}
			/* else nothing to do */
		} else {
			to->pdd = from->pdd;
	...
	if (!ast_tvzero(from->end)) {
		if (!ast_tvzero(to->end)) {
			if (ast_tvcmp(to->end, from->end) < 0 ) {
				...
				to->rduration = (to->end.tv_sec - to->start.tv_sec)*1.0 + (to->end.tv_usec - to->start.tv_usec)/1000000.0;
				to->rbillsec = (to->end.tv_sec - to->answer.tv_sec)*1.0 + (to->end.tv_usec - to->answer.tv_usec)/1000000.0;
			}
			/* else, nothing to do */
		} else {
			to->end = from->end;
			...
			to->billsec = ast_tvzero(to->answer) ? 0 : to->end.tv_sec - to->answer.tv_sec;
			to->rduration = (to->end.tv_sec - to->start.tv_sec)*1.0 + (to->end.tv_usec - to->start.tv_usec)/1000000.0;
			to->rbillsec = ast_tvzero(to->answer) ? 0 : (to->end.tv_sec - to->answer.tv_sec)*1.0 + (to->end.tv_usec - to->answer.tv_usec)/1000000.0;
		}
	}
...
void ast_cdr_pdd(struct ast_cdr *cdr)
{
	for (; cdr; cdr = cdr->next) {
		if (!ast_test_flag(cdr, AST_CDR_FLAG_LOCKED)) {
			check_post(cdr);
...
void ast_cdr_end(struct ast_cdr *cdr)
{
	for ( ; cdr ; cdr = cdr->next) {
		...
		if (ast_tvzero(cdr->start)) {
			...
			cdr->rduration = (cdr->end.tv_sec - cdr->start.tv_sec)*1.0 + (cdr->end.tv_usec - cdr->start.tv_usec)/1000000.0;
			
			if (!ast_tvzero(cdr->pdd)){
		...
		if (ast_tvzero(cdr->answer)) {
			...
			cdr->rbillsec = (cdr->end.tv_sec - cdr->answer.tv_sec)*1.0 + (cdr->end.tv_usec - cdr->answer.tv_usec)/1000000.0;
		}
	}
}
...
void ast_cdr_reset(struct ast_cdr *cdr, struct ast_flags *_flags)
{
	...
	for ( ; cdr ; cdr = cdr->next) {
		...
		if (ast_test_flag(&flags, AST_CDR_FLAG_LOCKED) || !ast_test_flag(cdr, AST_CDR_FLAG_LOCKED)) {
			...
			memset(&cdr->pdd, 0, sizeof(cdr->pdd));
			cdr->pddsec = 0;
			cdr->billsec = 0;
			cdr->duration = 0;
			cdr->rbillsec = 0;
