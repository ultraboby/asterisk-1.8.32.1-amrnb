...
struct ast_flags ast_channel_options = {0}; /* Channel options such as octet alignment */
...
static int ast_amr_reload(void)
{
	struct ast_variable *var;
	struct ast_flags config_flags = { 0 };
	struct ast_config *cfg = ast_config_load("codecs.conf", config_flags);
	...
	for (var = ast_variable_browse(cfg, "amr"); var; var = var->next) {
		if (!strcasecmp(var->name, "octet-aligned")) {
			ast_set2_flag(&ast_channel_options, ast_true(var->value), AST_CHANNEL_OPT_FLAG_OCTET_ALIGN);
		}
	}
