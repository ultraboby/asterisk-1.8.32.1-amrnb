			...
			case AST_FRAME_CONTROL:
				...
				case AST_CONTROL_ANSWER:
					...
					if (!pdd_set) {
						pdd_set = 1;
						ast_cdr_pdd(in->cdr);
						pdd = (float)((in->cdr->pdd.tv_sec - in->cdr->start.tv_sec)*1.0 + (in->cdr->pdd.tv_usec - in->cdr->start.tv_usec)/1000000.0 );
						ast_verbose( VERBOSE_PREFIX_3 "%s PDD is %f\n", o->chan->name, pdd);
						o->chan->cdr->pddsec = pdd;
					}
